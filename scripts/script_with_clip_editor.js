...
function updateCurrentLayer() {
    pushHistory('updateCurrentLayer');
...
function renderAnimationControls() {
    ...
    help.style.margin = '0.25rem 0 0.75rem 0';
    help.textContent = 'Tip: stack multiple animations. Each item can be collapsed. Use easing + direction for finer tuning.';
    ...
    function numberInput(value, { min = null, max = null, step = 'any', onChange } = {}) {
    ...
    layer.animations.forEach((anim, index) => {
        ...
        // Defaults for new fine-tuning params
        if (anim.enabled === undefined) anim.enabled = true;
        if (anim.easing === undefined) anim.easing = 'linear';
        if (anim.direction === undefined) anim.direction = anim.reverse ? 'reverse' : 'normal';
        if (anim.delay === undefined) anim.delay = 0;
        if (anim.iterations === undefined) anim.iterations = 'infinite';
        ...
        enabled.checked = !!anim.enabled;
        enabled.title = 'Enable/disable this animation';
        enabled.onchange = (e) => {
            e.stopPropagation();
            pushHistory('toggle animation');
            updateAnimation(index, 'enabled', enabled.checked);
        };

        const title = document.createElement('div');
        title.style.fontWeight = '600';
        title.textContent = anim.type || 'animation';
        ...
        body.style.padding = '0.5rem 0 0.25rem 0';

        // Type
        const typeSel = selectInput(anim.type || 'rotate', [
            { value: 'rotate', label: 'Rotate' },
            { value: 'step-rotate', label: 'Step Rotate' },
            { value: 'translate', label: 'Translate' },
            { value: 'scale', label: 'Scale' },
            { value: 'skew', label: 'Skew' },
            { value: 'slide', label: 'Slide (background)' },
            { value: 'pulse', label: 'Pulse (opacity)' },
            { value: 'hue', label: 'Hue Shift' },
            { value: 'blur', label: 'Blur' },
            { value: 'saturation', label: 'Saturation' },
            { value: 'contrast', label: 'Contrast' },
            { value: 'dropglow', label: 'Drop Glow' },
            { value: 'morph', label: 'Morph (advanced)' }
        ], (val) => {
            pushHistory('change animation type');
            updateAnimation(index, 'type', val);
        });
        body.appendChild(row('Type', typeSel));

        // Timing
        body.appendChild(row('Duration (s)', numberInput(anim.duration ?? defaultDuration, {
            min: 0.1, step: 0.1,
            onChange: (v) => { pushHistory('duration'); updateAnimation(index, 'duration', v); }
        })));

        body.appendChild(row('Delay (s)', numberInput(anim.delay ?? 0, {
            min: 0, step: 0.1,
            onChange: (v) => { pushHistory('delay'); updateAnimation(index, 'delay', v); }
        })));

        body.appendChild(row('Easing', selectInput(anim.easing ?? 'linear', easingOptions, (v) => {
            pushHistory('easing');
            updateAnimation(index, 'easing', v);
        })));

        body.appendChild(row('Direction', selectInput(anim.direction ?? 'normal', [
            ...
            updateAnimation(index, 'direction', v);
        })));

        const iterWrap = document.createElement('div');
        iterWrap.style.display = 'flex';
        iterWrap.style.gap = '8px';
        const iterSel = selectInput(String(anim.iterations ?? 'infinite'), [
            ...
            if (v === 'custom') {
                const custom = prompt('Iterations (number or "infinite"):', String(anim.iterations ?? 'infinite'));
                if (custom !== null) {
                    pushHistory('iterations');
                    updateAnimation(index, 'iterations', custom.trim());
                } else {
                    renderAnimationControls();
                }
                return;
            ...
            pushHistory('iterations');
            updateAnimation(index, 'iterations', v);
        });
        iterWrap.appendChild(iterSel);
        body.appendChild(row('Iterations', iterWrap));

        // Type-specific fine tuning
        const type = anim.type || 'rotate';

        if (type === 'rotate') {
            // simple reverse toggle kept for backward compatibility
            const chk = document.createElement('input');
            chk.type = 'checkbox';
            chk.checked = !!anim.reverse;
            chk.onchange = () => { pushHistory('reverse'); updateAnimation(index, 'reverse', chk.checked); };
            body.appendChild(row('Legacy reverse', chk));
        }

        if (type === 'step-rotate') {
            body.appendChild(row('Steps', numberInput(anim.step ?? 8, {
                min: 2, step: 1,
                onChange: (v) => { pushHistory('steps'); updateAnimation(index, 'step', v); }
            })));
        }

        if (type === 'translate') {
            body.appendChild(row('X from (px)', numberInput(anim.transXfrom ?? 0, { step: 1, onChange: (v) => { pushHistory(); updateAnimation(index, 'transXfrom', v); } })));
            body.appendChild(row('Y from (px)', numberInput(anim.transYfrom ?? 0, { step: 1, onChange: (v) => { pushHistory(); updateAnimation(index, 'transYfrom', v); } })));
            body.appendChild(row('X to (px)', numberInput(anim.transXto ?? 10, { step: 1, onChange: (v) => { pushHistory(); updateAnimation(index, 'transXto', v); } })));
            body.appendChild(row('Y to (px)', numberInput(anim.transYto ?? 10, { step: 1, onChange: (v) => { pushHistory(); updateAnimation(index, 'transYto', v); } })));
        }

        if (type === 'scale') {
            body.appendChild(row('From X', numberInput(anim.scalefromX ?? 1, { step: 0.01, onChange: (v) => { pushHistory(); updateAnimation(index, 'scalefromX', v); } })));
            body.appendChild(row('From Y', numberInput(anim.scalefromY ?? 1, { step: 0.01, onChange: (v) => { pushHistory(); updateAnimation(index, 'scalefromY', v); } })));
            body.appendChild(row('To X', numberInput(anim.scaletoX ?? 1.1, { step: 0.01, onChange: (v) => { pushHistory(); updateAnimation(index, 'scaletoX', v); } })));
            body.appendChild(row('To Y', numberInput(anim.scaletoY ?? 1.1, { step: 0.01, onChange: (v) => { pushHistory(); updateAnimation(index, 'scaletoY', v); } })));
        }

        if (type === 'skew') {
            body.appendChild(row('Direction', selectInput(anim.direction || 'x', [
                { value: 'x', label: 'X' },
                { value: 'y', label: 'Y' }
            ], (v) => { pushHistory(); updateAnimation(index, 'direction', v); })));
            body.appendChild(row('Intensity (deg)', numberInput(anim.intensity ?? 10, { step: 1, onChange: (v) => { pushHistory(); updateAnimation(index, 'intensity', v); } })));
        }

        if (type === 'hue') {
            body.appendChild(row('From Hue', numberInput(anim.fromHue ?? 0, { step: 1, onChange: (v) => { pushHistory(); updateAnimation(index, 'fromHue', v); } })));
            body.appendChild(row('To Hue', numberInput(anim.toHue ?? 360, { step: 1, onChange: (v) => { pushHistory(); updateAnimation(index, 'toHue', v); } })));
        }

        if (type === 'blur') {
            body.appendChild(row('Blur from (px)', numberInput(anim.blurfrom ?? 0, { step: 0.5, onChange: (v) => { pushHistory(); updateAnimation(index, 'blurfrom', v); } })));
            body.appendChild(row('Blur to (px)', numberInput(anim.blurto ?? 6, { step: 0.5, onChange: (v) => { pushHistory(); updateAnimation(index, 'blurto', v); } })));
        }

        if (type === 'saturation') {
            body.appendChild(row('Sat from', numberInput(anim.satfrom ?? 1, { step: 0.05, onChange: (v) => { pushHistory(); updateAnimation(index, 'satfrom', v); } })));
            body.appendChild(row('Sat to', numberInput(anim.satto ?? 2, { step: 0.05, onChange: (v) => { pushHistory(); updateAnimation(index, 'satto', v); } })));
        }

        if (type === 'contrast') {
            body.appendChild(row('Contrast from', numberInput(anim.contrastfrom ?? 1, { step: 0.05, onChange: (v) => { pushHistory(); updateAnimation(index, 'contrastfrom', v); } })));
            body.appendChild(row('Contrast to', numberInput(anim.contrastto ?? 1.6, { step: 0.05, onChange: (v) => { pushHistory(); updateAnimation(index, 'contrastto', v); } })));
        }
        ...
        if (type === 'slide') {
            body.appendChild(row('Direction', selectInput(anim.direction || 'horizontal', [
        ...
        if (type === 'morph') {
            ...
            note.textContent = 'Morph is a power-user combo animation. It will respect the fields you set on the animation object (hue/blur/scale/translate/etc.).';
...
function updateAnimation(index, key, value) {
    if (currentLayerIndex < 0) return;
    const layer = layers[currentLayerIndex];
    if (!layer || !layer.animations || !layer.animations[index]) return;
    ...
    if (key === 'reverse' || key === 'enabled') value = Boolean(value);
